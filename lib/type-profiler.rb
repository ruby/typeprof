...
module TypeProfiler
  def self.type_profile(iseq)
    ...
    prologue_ctx = Context.new(nil, nil, Signature.new(:top, nil, nil, FormalArguments.new([], nil, nil, nil, nil, nil)))
  ...
  def self.starting_state(iseq)
    ...
    ctx = Context.new(iseq, cref, Signature.new(recv, nil, nil, FormalArguments.new([], nil, nil, nil, nil, _nil)))

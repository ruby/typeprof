...
「型プロファイラ」という名前は変わるかもしれません。
この文書の最後に、いま提案された名前一覧を置いておきます。
...
* 一部のメソッドの型を指定した app.rbs とともに app.rb を解析する場合
...
主に型プロファイラのデバッグ用途として、次の環境変数を見るようになっています。

* `TP_DETAILED_STUB=1`: デフォルトでは`A | untyped`と推定されたところを単に`A`と出力しますが、より生の出力、つまり`A | untyped`と出力します。
* `TP_SHOW_ERRORS=1`: 実行中に見つけたバグの可能性を出力します（多くの場合、大量のfalse positiveが出ます）。
* `TP_DEBUG=1`: デバッグ出力を有効にします。


## 型プロファイラとは

型プロファイラは、Rubyプログラムを型レベルで抽象的に実行するインタプリタです。
...
これらの実行結果の観察を集めて、型プロファイラは「メソッド`foo`は`Integer`を受け取り、`String`を返す」という情報をRBSの形式で出力します。
また、`p`の引数は`Revealed types`として出力されます。

インスタンス変数は、通常のRubyではオブジェクトごとに記憶される変数ですが、型プロファイラではクラス単位に集約されます。
...
前述の通り、型プロファイラはRubyの値を型のようなレベルに抽象化して扱います。
ただし、クラスオブジェクトなど、一部の値は抽象化しません。
紛らわしいので、型プロファイラが使う抽象化された値のことを「抽象値」と呼びます。

型プロファイラが扱う抽象値は次のとおりです。
...
型プロファイラは型レベルのRubyインタプリタと言いましたが、その実行順はRubyのものとはまったく異なります。
...
型プロファイラは理論上の制限や実装上の制限により、プログラマの意図を正しく推定できない場合があります。
そのような場合、部分的にRBSを手書きし、型プロファイラに意図を伝えることができます。

たとえば型プロファイラはオーバーロードを推定できないので、次のようなコードを解析すると少し雑な推定となります。

```
# プログラマの意図：     (Integer) -> Integer | (String) -> String
# 型プロファイラの推定： (Integer | String) -> (Integer | String)
def foo(n)
  if n.is_a?(Integer)
    42
  else
...
型プロファイラの挙動・解析結果を理解するのはなかなか難しい問題です。
